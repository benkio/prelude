...
*** gptel
Interact with Cursor CLI (agent) from Emacs via a local proxy.
Start the proxy first: ~node ~/.emacs.d/personal/cursor-cli-proxy.js~ (or run in background).

Not native support for cursor-cli. Just installing it for now. Setup later
...
#+begin_src emacs-lisp
  (use-package agent-shell
    :config
    ;; Keep PATH in sync so `agent` and `opencode` are discoverable.
    ;; (let ((opencode-bin (expand-file-name "~/.opencode/bin")))
    ;;   (add-to-list 'exec-path opencode-bin)
    ;;   (unless (string-match-p (regexp-quote opencode-bin) (or (getenv "PATH") ""))
