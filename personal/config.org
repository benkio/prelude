...
3. hs-minor-mode in prog modes
4. autorevert mode global
...
Install grammars under =~/.emacs.d/tree-sitter/= and register download recipes.  This runs whenever =treesit= is first loaded (for example when opening TypeScript), not only when Nix mode is used.

#+begin_src emacs-lisp
  (with-eval-after-load 'treesit
    (let ((treesit-dir (expand-file-name "tree-sitter" user-emacs-directory)))
      (unless (file-directory-p treesit-dir)
        (make-directory treesit-dir t))
      (add-to-list 'treesit-extra-load-path treesit-dir))
    ;; `treesit-language-source-alist` is nil by default; recipes are required
    ;; for `M-x treesit-install-language-grammar`.
    (dolist (recipe '((bash "https://github.com/tree-sitter/tree-sitter-bash")
    ...
    (dolist (gram '(nix typescript tsx gotmpl))
      (unless (treesit-language-available-p gram)
        (condition-case err
            (treesit-install-language-grammar gram)
          (error
           (message "treesit: failed installing %s grammar: %s" gram err))))))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (setq initial-scratch-message "卐")
...
- Dired displaying ls
- Font
- ~projectile-switch-project-action~ to ~projectile-vc~
#+begin_src emacs-lisp
  (require 'org)
  ;; Org defaults to `typescript-mode` (typescript.el); use built-in TS modes.
  ...
  (setq org-capture-templates
        '(("t" "Todo" entry (file+headline "~/Dropbox/notes/TODO.org" "TODOs")
           "* TODO %?\n  %i\n")
          ("d" "Download" entry (file+olp "~/Dropbox/notes/TVShows.org" "TV Shows" "Capture")
           "* %?\n  %i\n")
...
*** agent-shell
Native ACP shell integration for Cursor and OpenCode CLIs.

#+begin_src emacs-lisp
  (use-package agent-shell
    :config
    ;; Keep PATH/exec-path in sync so `agent` is discoverable.
...
In particular it enables ~dired-x~ and ~dired-omit-mode~
...
#+begin_src emacs-lisp
  ...
  (use-package dired-git-info
    ...
    :bind (:map dired-mode-map
                (")" . dired-git-info-mode)));; hide .dot files by default
#+end_src
...
#+begin_src emacs-lisp
  (use-package indent-guide
    :config
    ;; Emacs 31 can call this helper before `define-minor-mode` creates it
    ;; when stale autoload/native-cache code is around.
    (defvar-local indent-guide-mode--set-explicitly nil)
    (dolist (mode '(org-mode org-agenda-mode outline-mode
                            rg-mode compilation-mode))
      (add-to-list 'indent-guide-inhibit-modes mode))
    (indent-guide-global-mode)
    (defun my/indent-guide--org-buffer-p ()
      "Non-nil when indent guides must not run (org + virtual indent)."
      (or (derived-mode-p 'org-mode 'org-agenda-mode 'outline-mode)
          (bound-and-true-p org-indent-mode)))
    (defun my/indent-guide-mode--guard (orig &optional arg)
      "Never enable indent-guide in org buffers."
      (unless (my/indent-guide--org-buffer-p)
        (funcall orig arg)))
    (advice-add #'indent-guide-mode :around #'my/indent-guide-mode--guard)
    (defun my/indent-guide-show--guard (orig &rest args)
      (unless (my/indent-guide--org-buffer-p)
        (condition-case err
            (apply orig args)
          (error
           (indent-guide-remove)))))
    (advice-add #'indent-guide-show :around #'my/indent-guide-show--guard)
    (defun my/indent-guide-disable-in-org ()
      (indent-guide-mode -1)
      (indent-guide-remove))
    (add-hook 'org-mode-hook #'my/indent-guide-disable-in-org))
#+end_src
** Text Maniputanion
*** Iedit
...
#+begin_src emacs-lisp
  (use-package nix-ts-mode
    :mode "\\.nix\\'"
    :config
    ;; If some package selects `nix-mode`, prefer the tree-sitter mode.
  ...
  (with-eval-after-load 'treesit
    (defvar gotmpl-ts-mode--font-lock-settings
      (treesit-font-lock-rules
       ...
       :language 'gotmpl
       :feature 'keyword
       '(["if" "else" "range" "with" "end" "template" "define" "block"] @font-lock-keyword-face)
       :language 'gotmpl
       :feature 'string
       ...
       '(("{{" @font-lock-bracket-face)
         ...
         ("," @font-lock-delimiter-face)))
      "Tree-sitter font-lock settings for `gotmpl-ts-mode`.")

    (define-derived-mode gotmpl-ts-mode prog-mode "GoTemplate[TS]"
      "Major mode for Go templates powered by tree-sitter."
      (unless (treesit-ready-p 'gotmpl t)
        (error "Tree-sitter grammar for `gotmpl` is not available"))
      (treesit-parser-create 'gotmpl)
      (setq-local treesit-font-lock-settings gotmpl-ts-mode--font-lock-settings)
      (setq-local treesit-font-lock-feature-list
                  '((comment keyword string)
                    (number variable function property)
                    (operator delimiter)))
      (setq-local comment-start "{{/* ")
      (setq-local comment-end " */}}")
      (treesit-major-mode-setup))

    ;; Prelude maps `*.tpl` to `web-mode`; force Go template files to use
    ;; tree-sitter mode by adding this rule at the front.
    (add-to-list 'auto-mode-alist '("\\.tpl\\'" . gotmpl-ts-mode)))
#+end_src
**** Mermaid
...
(use-package sbt-mode
  ...
  :config
  (defvar benkio/sbt-rgrep-use-rg nil
    "When non-nil, translate grep BRE regexps for ripgrep.")

  (defconst benkio/sbt-rg-find-template
    "rg --no-heading --line-number --color never <C> -e <R> <D>"
    "Use ripgrep for `sbt-mode' recursive searches.")

  (defun benkio/sbt-bre-to-rg-regexp (regexp)
    "Translate grep BRE escapes in REGEXP to rg syntax."
    (let ((re regexp))
      (setq re (replace-regexp-in-string (regexp-quote "\\(") "(" re t t))
      (setq re (replace-regexp-in-string (regexp-quote "\\)") ")" re t t))
      (setq re (replace-regexp-in-string (regexp-quote "\\|") "|" re t t))
      (setq re (replace-regexp-in-string (regexp-quote "\\+") "+" re t t))
      re))

  (defun benkio/sbt-rgrep-use-rg-regexp (fn regexp &optional files dir confirm)
    "Convert sbt-mode grep regexps before calling FN."
    (if benkio/sbt-rgrep-use-rg
        (funcall fn (benkio/sbt-bre-to-rg-regexp regexp) files dir confirm)
      (funcall fn regexp files dir confirm)))

  (defun benkio/sbt-use-rg-for-rgrep (fn &rest args)
    "Run sbt rgrep helpers with ripgrep instead of grep."
    (let ((grep-find-template benkio/sbt-rg-find-template)
          (benkio/sbt-rgrep-use-rg t))
      (apply fn args)))

  (unless (advice-member-p #'benkio/sbt-rgrep-use-rg-regexp 'rgrep)
    (advice-add 'rgrep :around #'benkio/sbt-rgrep-use-rg-regexp))
  (unless (advice-member-p #'benkio/sbt-use-rg-for-rgrep 'sbt-grep)
    (advice-add 'sbt-grep :around #'benkio/sbt-use-rg-for-rgrep))
  (unless (advice-member-p #'benkio/sbt-use-rg-for-rgrep 'sbt-find-usages)
    (advice-add 'sbt-find-usages :around #'benkio/sbt-use-rg-for-rgrep))
  (unless (advice-member-p #'benkio/sbt-use-rg-for-rgrep 'sbt-find-definitions)
    (advice-add 'sbt-find-definitions :around #'benkio/sbt-use-rg-for-rgrep))
...
#+begin_src emacs-lisp
  (use-package ox-reveal
    ...
    :config
    (require 'ox-reveal)
    ;; npm layout (dist/, plugin/); do not append /reveal.js. cdnjs 5.x layout differs from ox-reveal.
    (setq org-reveal-root "https://cdn.jsdelivr.net/npm/reveal.js@5.2.1"
...
#+begin_src emacs-lisp
(defun org-calendar-cursor-show (orig-fun &rest args)
  (when (equal (car args) '(setq cursor-type nil))
    (setcar args '(setq cursor-type 'box)))
  (apply orig-fun args))

(advice-add 'org-eval-in-calendar :around #'org-calendar-cursor-show)
#+end_src
        ...
        (progn
          ...
          (let ((urls (or opt-urls
            ...
            (let ((directory (expand-file-name (read-directory-name "Select output directory: "))))
              ...
              (let* ((url-list (seq-remove #'s-blank?
                                           (mapcar #'s-trim
                                                   (split-string (s-trim urls) "[\r\n]+" t))))
                    (aria2Command (concat "aria2c -V --seed-ratio=0.0 --seed-time=144000 -d "
                                          (shell-quote-argument directory) " -Z "
                                          (mapconcat #'shell-quote-argument url-list " "))))
...
#+begin_src emacs-lisp
    ...
    (defvar my/mega-local-sync-dir (expand-file-name "~/Mega")
      "Local directory synchronized with Mega.")

    (defvar my/mega-remote-sync-dir "/"
      "Remote Mega directory mirroring `my/mega-local-sync-dir'.")

    (defvar my/mega-max-file-size-bytes (* 250 1024 1024)
      "Maximum file size accepted by Mega link helper.
  If a file is bigger than this value, the command aborts.")

    (defun my/mega--first-link-from-output (output)
      "Extract the first Mega link from OUTPUT."
      (when (string-match "https?://mega\\.[^[:space:]]+" output)
        (match-string 0 output)))

  (defun my/mega--run-command (command &optional tolerated-error-regexp)
      "Run COMMAND and return its output.
  Raise an error including stderr when COMMAND fails."
      (with-temp-buffer
        (let ((exit-code (shell-command command (current-buffer) (current-buffer)))
              (output nil))
          (setq output (s-trim (buffer-string)))
        ;; Some `mega-export` builds can return non-zero while still printing a
        ;; valid link (e.g. when an export already exists). Accept that case.
        (unless (or (zerop exit-code)
                    (my/mega--first-link-from-output output)
                    (and tolerated-error-regexp
                         (string-match-p tolerated-error-regexp output)))
            (error "Command failed (exit %s): %s\n%s"
                   exit-code command
                   (if (s-blank? output) "(no output)" output)))
          output)))

    (defun get-or-create-mega-link (file)
      "Given an input FILE in the Mega-synced folder, return its Mega link.
  If FILE is larger than `my/mega-max-file-size-bytes', abort."
      (unless (executable-find "mega-export") (error "Please install mega-cmd (mega-export)"))
      (let* ((absolute-file (expand-file-name file))
             (file-size (file-attribute-size (file-attributes absolute-file)))
             (local-root (file-name-as-directory (expand-file-name my/mega-local-sync-dir)))
             (relative-path (file-relative-name absolute-file local-root)))
        (when (string-prefix-p ".." relative-path)
          (error "File is not inside synced dir %s: %s" local-root absolute-file))
        (when (> file-size my/mega-max-file-size-bytes)
          (error "File too large (%s bytes > %s bytes), aborting"
                 file-size my/mega-max-file-size-bytes))
        (let* ((remote-path (concat (file-name-as-directory my/mega-remote-sync-dir) relative-path))
               (get-command (format "mega-export %s" (shell-quote-argument remote-path)))
               (create-command (format "mega-export -a -f %s" (shell-quote-argument remote-path)))
             (existing-output (my/mega--run-command get-command "is not exported\\. Use -a to export it"))
               (existing-link (my/mega--first-link-from-output existing-output)))
          (if existing-link
              (progn
                (message "Get Mega Link for %s: %s" file existing-link)
                existing-link)
            (progn
              (message "Create Mega Link for %s" file)
              (let* ((created-output (my/mega--run-command create-command))
                     (created-link (my/mega--first-link-from-output created-output)))
                (unless created-link
                  (error "Mega link not found after creation for: %s" file))
                created-link))))))

    (defun get-or-create-mega-links ()
      "Given marked files in dired, copy lines in filename=mega_link format."
      (interactive)
      (let* ((entries (seq-map
                       (lambda (filename)
                         (let* ((link (get-or-create-mega-link filename))
                                (basename (file-name-nondirectory filename)))
                           (format "%s=%s" basename link)))
                       (dired-get-marked-files)))
             (result (mapconcat #'identity entries "\n")))
        (with-temp-buffer-window (make-temp-name "mega-link-copy-") nil nil (princ result))))
#+end_src
