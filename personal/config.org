...
#+begin_src emacs-lisp
  (use-package nix-ts-mode
    ...
    (with-eval-after-load 'treesit
      ;; Ensure Emacs can find grammars installed under ~/.emacs.d/tree-sitter/
      ...
      ;; Tell Emacs where to fetch the Nix grammar from.
      (add-to-list 'treesit-language-source-alist
                   '(nix . ("https://github.com/nix-community/tree-sitter-nix")))

      ;; Auto-install the grammar if it’s missing (but don’t hard-error on startup).
      (unless (treesit-language-available-p 'nix)
        (condition-case err
            (treesit-install-language-grammar 'nix)
          (error
           (message "treesit: failed installing nix grammar: %s" err)))))
