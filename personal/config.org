...
#+begin_src emacs-lisp
  ...
  (defvar my/mega-remote-sync-dir "/Root/Mega"
...
If a file is bigger than this value, the command aborts.")
  ...
  (defun my/mega--first-link-from-output (output)
    ...
    (car (seq-filter (lambda (line) (string-match-p "^https://mega\\.nz/" line))
                     (split-string (s-trim output) "\n" t))))
...
If FILE is larger than `my/mega-max-file-size-bytes', abort."
    (unless (executable-find "megatools") (error "Please install megatools"))
    (let* ((absolute-file (expand-file-name file))
      ...
      (let* ((remote-path (concat (file-name-as-directory my/mega-remote-sync-dir) relative-path))
             (get-command (format "megatools export %s" (shell-quote-argument remote-path)))
             (create-command (format "megatools export -a %s" (shell-quote-argument remote-path)))
             (existing-output (shell-command-to-string get-command))
        ...
        (if existing-link
          ...
          (progn
            ...
            (shell-command create-command)
            (let ((created-link (my/mega--first-link-from-output (shell-command-to-string get-command))))
              (unless created-link
